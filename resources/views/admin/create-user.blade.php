    ...
    <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="nom"
                ...
                required value="{{ old('nom') }}" placeholder="Nom">
        </div>
        ...
        <div>
            ...
            <input type="text" name="prenom"
                class="w-full border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-violet-400 transition"
                required value="{{ old('prenom') }}" placeholder="Prénom">
        </div>
        ...
        <div>
            ...
            <input type="email" name="email"
                class="w-full border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-violet-400 transition"
                required value="{{ old('email') }}" placeholder="exemple@domaine.com">
        </div>
        ...
        <div>
            ...
            <select name="categorie" id="roleSelect"
                ...
                @foreach($roles as $role)
                    <option value="{{ $role }}" {{ old('categorie') == $role ? 'selected' : '' }}>
                        {{ ucfirst($role) }}
                    </option>
                @endforeach
            </select>
...
<div id="etudiantFieldParent" style="display: none;">
    <label class="block mb-1 text-sm font-semibold text-gray-700">Étudiant associé</label>
...
function toggleFields() {
    ...
    const etudiantFieldParent = document.getElementById('etudiantFieldParent');

    // Masquer tous les champs
    classeFieldEtudiant.style.display = 'none';
    classeFieldCoordinateur.style.display = 'none';
    etudiantFieldParent.style.display = 'none';
    ...
    if (roleSelect.value === 'etudiant') {
        ...
        etudiantFieldParent.style.display = 'block';
    }
}
